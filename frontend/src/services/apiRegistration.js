...
export async function saveRegDetails(formData){
    ...
    const response = await axios.post('http://localhost:3000/api/registration/savedata',
    formData,
    {
        headers:{
...
export async function uploadSignature(formData){
    const token = JSON.parse(localStorage.getItem('token'));
    const response = await axios.post('http://localhost:3000/api/registration/upload-signature',
    formData,
    {
        headers:{
            'Authorization': 'Bearer '+`${token.token}`
...
export async function uploadExtra(formData){
    const token = JSON.parse(localStorage.getItem('token'));
    const response = await axios.post('http://localhost:3000/api/registration/extra-upload',
    formData,
    {
        headers:{
            'Authorization': 'Bearer '+`${token.token}`
